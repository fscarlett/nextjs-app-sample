...
import SubcatBarElementNoLabel from './SubcatBarElementNoLabel'
import styles from '@/styles/GroupBarChart.module.css'

export default function GroupBarChartSummary({ ig, proj }) {
        ...
        <div className={styles.group_detail_link}>
          ...
          <div className={styles.group_barchart_info_wrapper}>
            ...
            <div className={styles.subcat_bars_wrapper}>
              {subcats.map((sub) => (
                <SubcatBarElementNoLabel key={sub.id} subcat={sub} />
              ))}
            </div>
          </div>
        </div>
